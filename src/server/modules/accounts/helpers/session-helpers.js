...
/**
 * Build grouped areas for PSO responsibility
 * @private
 */
...
/**
 * Build grouped areas for RMA responsibility
 * @private
 */
  ...
  psoAreas
    ...
    .forEach((pso) => {
      ...
      if (eaParent && children.length > 0) {
        groups.push({ parent: pso, eaParent, children })
      }
    })
...
export function buildGroupedAreas(
  ...
  if (responsibility === RESPONSIBILITY_MAP.RMA) {
    return buildRMAGroupedAreas(masterAreas, sessionData, excludeAreaId)
  }
  return null
}
