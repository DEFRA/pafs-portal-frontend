...
async function executeRequest(url, options, controller) {
  ...
  // For 401 errors, check if there's an errorCode in the response
  // This handles JWT validation failures with specific error codes
  if (response.status === statusCodes.unauthorized && data?.errorCode) {
    return {
      success: false,
      status: response.status,
      errors: [{ errorCode: data.errorCode }],
