...
import i18nService, { translate, I18nService } from './index.js'
...
vi.mock('../../../../config/config.js', () => ({
  config: {
    get: vi.fn((key) => {
      ...
      if (key === 'logConfig.enabled') return true
      if (key === 'logConfig.level') return 'info'
      if (key === 'logConfig.format') return 'json'
      if (key === 'logConfig.redact') return []
      return null
    })
  }
}))
...
describe('i18n translation service with fs-backend', () => {
  beforeAll(async () => {
    // Initialize i18n service before running tests
    // fs-backend will load translation files from the actual file system
  ...
  describe('basic translation', () => {
    ...
    test('handles deeply nested translation keys', () => {
      expect(translate('home.title')).toBe('Your Proposals')
    })
  })
...
describe('i18n Hapi plugin', () => {
  ...
  test('registers as a Hapi plugin', async () => {
    const { i18nPlugin } = await import('./index.js')
